...
class SekaiClient:
    def __init__(
        ...
        account: Union[SekaiAccountCP, SekaiAccountNuverse],
        key: bytes,
        iv: bytes,
        cookie_helper: Optional[SekaiCookieHelper] = None,
    ...
    async def _response(self, response: ClientResponse) -> Optional[Tuple[Dict, int]]:
        if response.content_type in ["application/octet-stream", "binary/octet-stream"]:
            ...
            if response.status in [
                SekaiApiHttpStatus.OK,
                SekaiApiHttpStatus.CLIENT_ERROR,
                SekaiApiHttpStatus.NOT_FOUND,
                SekaiApiHttpStatus.CONFLICT,
            ]:
    ...
    async def call_api(
        ...
        path: str,
        method: str = "GET",
        data: Optional[Union[bytes, Dict, List]] = None,
