...
- `src/updater/` - Cron jobs for master data version check/download (with local or remote-borrowed accounts), git push; app identity is not polled — it arrives via `POST /internal/app-identity` (`src/api/internal.rs`); `sync.rs` pulls master bundles from a region's owner node; `master_stream.rs` decodes a downloaded master payload table by table (rows streamed) so the producer's peak memory is bounded by one row, not the payload — keep new master consumers on that path, never `unpack_ordered` on a whole master
